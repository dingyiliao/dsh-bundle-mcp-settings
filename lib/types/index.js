...
import { McpSettingsRuntime } from "./runtime.js";
...
export function assertServiceableMcpSettings(config) {
    for (const [serverName, server] of Object.entries(config.servers)) {
        ...
        if (server.transport === 'stdio') {
            if (server.command.trim() === '')
                throw new Error(`mcp: stdio server "${serverName}" requires command`);
            continue;
        }
        let endpoint;
        try {
...
export async function apply(ctx, config) {
    let current = () => config;
    const runtime = new McpSettingsRuntime(server => ctx.plugin(McpClient, server), (serverName, error) => {
